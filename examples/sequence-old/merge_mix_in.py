...
# that is a mix of new data and existing sequences.
#
...
# We will mix this data into the other two sequences
device = "numpy"  # Note: this works with torch devices as well
data = {
    "Scene-001": {
        "0000000000": File('../data/scene1/0000-occ.png').read(dtype=itypes.bool, device=device, dims="bchw"),
        "0000000001": File('../data/scene1/0001-occ.png').read(dtype=itypes.bool, device=device, dims="bchw"),
    },
    "Scene-002": {
        "0000000000": File('../data/scene2/0000-occ.png').read(dtype=itypes.bool, device=device, dims="bchw"),
    }
}
...
new_seq = Sequence(source_mode="copy")
...
with new_seq.grid.new_row() as row:
    row.skip_cell()
    row.skip_cell()
    row.add_cell('image', 'occ') # This is not sourced and will be mixed in

# We create a new sequence filling in only the occlusions.
# Soruced ids will automatically be filled in.
for scene_name, frames in data.items():
    scene = new_seq.data.scene(scene_name)
    for frame_name, occ_data in frames.items():
        frame = scene.sample(frame_name)
        frame.set_data("occ", occ_data)
...
new_seq.write(filename='out_merge_mix_in/data.gridseq')

print("To view run: \"iviz out_merge_mix_in/data.gridseq\"")
print()
